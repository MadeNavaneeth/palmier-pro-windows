...
| PRs #569–#572 | Agent generation model recommendations (Seedance 2.5, GPT Image, Seedream, MiniMax H3) | Planned | Agent system prompt text recommending generation models. `generate_media` is now live over the provider registry, so these are one-line catalog/prompt additions per model rather than blocked work |
