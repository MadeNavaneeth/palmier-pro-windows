...
| PRs #482, #494, #509, #507, #525, #529, #530, #519, #524 | Text styling cluster (default style centralization, preview rebuild preservation, alignment anchoring, fitted boxes, inverted fill, Gaussian blur, stencil mattes, perspective tilt, preset cleanup) | Partial | The text domain this cluster assumed absent has landed: `shared/editor/title.ts` owns sanitization plus a style model (size ratio, color, font family, bold, alignment, background with alpha, stroke width/color) rendered identically by canvas preview and FFmpeg drawtext, edited inline on the clip and from the Agent via `add_texts`/`set_title_text`. Of the nine upstream PRs: default-style centralization (#482) is covered by `DEFAULT_TITLE_STYLE`; alignment (#509) has its field and both-path plumbing; and **fitted boxes (#507) are in** — background padding is a style field (`titleBackgroundPadding`, agent-exposed 0-128 px) driving canvas rects inflated symmetrically and drawtext's native `boxborderw`, with the canvas now setting its font *before* measuring so both paths wrap the same glyphs and agree on box size (previously the measure ran under the default font and the vertical padding was asymmetric). Still missing: inverted fill (#525), Gaussian blur (#529), stencil mattes (#530), perspective tilt (#519), preview-rebuild preservation semantics (#494); preset cleanup is upstream-internal (#524) |
