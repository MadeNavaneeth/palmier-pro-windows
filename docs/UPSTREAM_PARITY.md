...
| Current `RippleEngine` / `EditorViewModel+Ripple` | Ripple editing and sync lock | Partial | Selected-clip and bounded-gap ripple delete plus linked edge ripple trim use shared atomic controller transactions, preserve sync-locked tracks, enforce edit locks/source limits/collision constraints, support one-step undo/redo, and are shared by UI/Agent/MCP; arbitrary range cutting and user-facing refusal diagnostics remain planned |
