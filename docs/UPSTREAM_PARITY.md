...
| PRs #570, #565, #568 | Generation cost display; inspect-frame grid overlay; static crop in set_clip_properties | Partial | **inspect_frame (#565) is live**: `{assetId, atSeconds, width?}` decodes through the shared frame decoder, encodes RGBA→PNG via a temp-file ffmpeg handoff, stores under userData/inspect-frames, and returns absolute path + dimensions + timecode; over MCP the PNG attaches as a native image content block so vision clients see the frame directly. **Static crop (#568) is live**: `Clip.crop` edge fractions (capped 0–0.45/pair ≤0.9 via `shared/media/source-crop.ts`) applied as a proportional sub-rect of the decoded RGBA in preview — pixel-equivalent to source-space crop through uniform scale — and an FFmpeg `crop` filter ahead of scale on export; agent tool `set_clip_crop` sanitizes and clears on all-zeros. Remaining from the trio: #570 generation cost display (needs provider cost metadata) |
| PRs #569â€“#572 | Agent generation model recommendations (Seedance 2.5, GPT Image, Seedream, MiniMax H3) | Planned | Agent system prompt text recommending generation models. `generate_media` is now live over the provider registry, so these are one-line catalog/prompt additions per model rather than blocked work |
