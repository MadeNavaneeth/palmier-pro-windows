...
| PRs #570, #565, #568 | Generation cost display; inspect-frame grid overlay; static crop in set_clip_properties | Planned | Three small upstream additions. #570 needs the generation UI surface (the tool itself is live; only cost display is missing). #565 requires upstream's inspect_frame tool which this port does not have. #568 adds crop fields to the clip model and compositor -- meaningful but needs its own scoping session alongside the R4 color work. None affect existing functionality |
