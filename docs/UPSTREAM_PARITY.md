...
- Upstream baseline: `eece9436fbc9d041443c19ce5d87a75e75a408e9`
- Baseline date: 2026-08-28 ("[build] Publish v0.8.1 appcast")
- Windows audit date: 2026-09-04 — `npm run upstream:audit:check` reports the
  tracked snapshot current; upstream `main` has not moved since the snapshot was
  written (60 open issues, 100 recent merged PRs captured)
- Upstream release at baseline: v0.8.1
- Previous baseline: `8805801fa4df8bc2dbc57cb0a854a1f5108f95c6` (v0.7.7+, 2026-08-24)
...
| #97 | Chroma key (green/blue screen removal) | Implemented | `shared/editor/chroma-key.ts` — per-clip key color/tolerance/softness/spill; preview applies a JS per-pixel pass on the decoded RGBA frame, export uses FFmpeg `colorkey`+`despill`, both reading the same clip fields; `set_clip_chroma_key` agent tool; Inspector swatch + sliders (`chroma-key.test.ts`, `executor.chroma-key.test.ts`) |
...
| PR #397, PR #438 | Multicam ripple synchronization and angle-trim steering | Planned | Multicam domain is not implemented. PR #438 is merged into a stacked branch (`trim/2-trim-clip-tool`) rather than `main`; it narrows the agent's tool description so an ordinary trim request stops being routed to the multicam angle trim. The transferable rule â€” a specialized tool must be reachable only on explicit intent, never as the default reading of a general request â€” already holds here because there is no specialized variant to mis-select |
...
| PR #369 | Edge rounding and softness | Implemented | `edgeRounding` and `edgeSoftness` (0–1 normalized) on Clip type; Canvas compositor uses offscreen canvas with roundRect clip + radial gradient alpha mask for preview; FFmpeg `geq` alpha expression for export; Inspector sliders for visual clips; `set_clip_edge_effects` agent tool; passthrough when both zero; `hasEdgeEffects` predicate + `buildEdgeGeqExpr` in `shared/editor/edge-effects.ts` |
...
| | PR #408 | Invert colors effect | Implemented | invertColors boolean on Clip type; toCanvasFilter renders invert(1); Inspector checkbox for visual clips; hasColorGrade includes invertColors; test updated |
| PR #406 | Source-video preparation and grouped video models | Partial | The registry is live: `generation/manager.ts` is the headless provider registry (fal, Replicate, and HiggsField registered) shared by the IPC shell and the Agent/MCP executor, and `generate_media` is wired end to end â€” configured-provider resolution with a precise no-key refusal, model defaults from the provider catalog, bounded timeout that cancels the request, and the finished file probed into the library as a first-class asset with provenance in the receipt (`manager.test.ts`, `executor.titles.test.ts` generate_media block). Still missing from upstream's vision: capability-grouped model metadata, duration limits per model, and shared source preprocessing |
...
| PR #410 | Additional 3x3 / 4x4 grid layouts | Partial | **The catalogue is now the full upstream one.** `shared/editor/grid-layout.ts` is the `VideoLayout` analogue: all 13 presets under upstream's raw values (`full`, `side_by_side`, `top_bottom`, the four `pip_*` corners, `grid_2x2/3x3/4x4`, `main_sidebar`, `three_up`, `three_stack`), each a list of named slots holding a normalized 0-1 canvas rect with a `z` for stacking, so the PiP inset draws over the main slot rather than under it. Slot ids match upstream (`main`, `left`/`right`, `top`/`bottom`, `inset`, `sidebar`, `center`, `middle`, row-major `rNcN`), as do the PiP constants (28% inset held 3.5% off its corner) and the 70/30 sidebar. Grids keep the single shared `generateGridCells` generator — upstream's `grid(rows:columns:)` — so tiling math cannot drift between them, and their floor-rounded pixel output is unchanged. `apply_layout` accepts the whole catalogue and reports the slots a preset defines; an unknown name is rejected with the valid list, matching upstream's error. One undo step per call (`grid-layout.test.ts`, `executor.layout.test.ts`). Remaining: upstream's `apply_layout` is slot-addressed (`slots: [{slot, mediaRef \| clipIds, anchor, anchorX/Y}]` with `fit: fill \| fit`, new-track creation for `mediaRef`, and refusals for same-track overlap and clips that never play together); here clips still map positionally to slots in draw order and always fill their cell. The timeline Layout menu still offers only the three grids — widening it is a UI change pending a rendered 1600x1000 / 1024x680 check |
| PR #411, PR #415, PR #416 | Seed Audio generation, promptless lip sync, AI video reframing, source-video normalization | Planned | The registry and `generate_media` tool are now live (see the PR #406 row), but these need capability-grouped model metadata, per-model duration limits, and reference/source input plumbing that upstream's grouped models carry â€” still adopted as catalog+schema extensions on top of the working registry |
| PRs #154, #289 | XML/FCPXML interchange and source timecode | Partial | Interchange is live over the Agent/MCP surface: `export_fcpxml {path}` writes FCPXML 1.11 (spine/lanes, audio roles, styled titles, decimal-second timing safe for fractional fps) and `import_fcpxml {path}` materializes a file additively â€” assets probed into the library with offline paths reported rather than failing, fresh tracks synthesized per lane, titles restored with style. Round-trip is test-pinned (`fcpxml/exporter.test.ts`, `importer.test.ts`, `executor.fcpxml.test.ts`). Remaining: effects/grades/blend representation (upstream's own scope is also partial here), a UI entry point alongside the agent tools, and source-timecode columns. **Fixed this pass:** `fileUrlToPath` stripped the leading slash from a POSIX `file:///` src, so every asset in a file written on Linux or macOS came back relative and read as offline on import — 0 clips placed, no error. Windows drive-letter URLs are unaffected, which is why the round-trip fixture (hard-coded `C:/media/...`) and the Windows-only CI never saw it. The decode now drops the empty-authority slash only in front of a drive letter, keeps a POSIX root, and keeps a `file://host/share` host as the root segment (`importer.test.ts` `fileUrlToPath` block, `executor.fcpxml.test.ts` round trip) |
...
| PR #426 | Configurable silence removal controls | Implemented | The controls are real and shared. `SILENCE_LIMITS` documents the accepted ranges (minimum pause 0.25-3 s, speech padding 0-0.5 s, matching upstream); `detectSilentRanges` applies speech padding only on a side that actually borders speech; the Inspector exposes Minimum Pause, Speech Padding, Threshold plus Reset, and the main process owns the saved values (`main/media/silence-settings.ts`) so a no-argument `remove_silence` performs exactly the edit the visible controls describe (`resolveSilenceConfig`; Agent and MCP included). The scoped half matches upstream's `clipIds` contract: `shared/editor/silence-scoping.ts` resolves an optional selection with upstream's exact refusal wordings, omitted scope sweeps every audio track in order, detection runs once per distinct source file, cuts go through the ripple engine so linked partners/sync lock/markers/undo ride along, missing sources refuse before any edit, and locked later tracks surface partial-success notes; the legacy single-`clipId` form is unchanged. The **timeline surface** closes the last gap: a Mark-Silence toggle shades detected dead-air spans over each audio clip's waveform via `silenceSpanRects` â€” the same mapping removal uses, so what is shaded is exactly what a cut removes â€” backed by a per-path span cache keyed to the saved controls (`silence-spans-cache.ts`, busted when a settings write confirms) and click-to-remove on one span through one undoable `rippleDeleteRanges` transaction, this port's rendering of upstream's remove-the-span-under-the-frame interaction. Deliberate absence: scoping to the marked in/out subrange has no upstream analogue in #426 and is not invented here. Detection measures an RMS envelope rather than an on-device speech mask, which is why Threshold is a user-facing control upstream does not have (`silence-detector.test.ts`, `silence-settings.test.ts`, `executor.silence.test.ts`, `executor.silence.scope.test.ts`, `silence-scoping.test.ts`, `silence-spans-cache.test.ts`) |
| PR #428 (open) | Take auditioning: track solo, comping, loop region | Implemented | **Track solo is live**: `toggleTrackSolo(trackId)` mutates an in-memory `soloed` flag on Track (never persisted, no undo entry); `activeTrackIds()` returns only soloed tracks when any are soloed; `visualClipsAtFrame`, `selectExportClips`, and `computeAudioPlan` all respect the solo filter. The solo button renders in TrackHeader with a headphones icon (yellow when active). Loop region is live: store loopEnabled state synced to PlaybackEngine.setLoopRange(), which wraps playback to the in/out range start instead of stopping at project end; toggle button with Repeat icon in the transport bar. Comping is live: compactTake(sourceClipId) takes the marked in/out range, finds or creates a top Comp video track, removes existing comp clips in the range, and places a trimmed copy of the source segment as one undoable step (shared/editor/controller.ts, compTrackId on Timeline, Y keybinding). The full #428 trio — solo, comping, and loop — is now implemented (`shared/editor/controller.ts`, `main/media/visible-clips.ts`, `shared/media/export-eligibility.ts`, `renderer/components/timeline/TrackHeader.tsx`, `controller.solo.test.ts`). Opened after the recorded baseline, so it is not in the snapshot's merged set. Fully applicable â€” none of the three exist here. The design constraint worth carrying over is that solo is *derived* state that never rewrites `muted`/`visible`, so un-soloing restores the user's exact setup and undo is unaffected; our `Track.visible` doubles as audio mute, which makes that separation more important, not less. Soloing a video take must follow `linkGroupId` to its audio, which this port already models. Export deliberately ignores solo upstream via a threaded flag; our exporter and preview share compositing rules, so the flag has to reach both or preview and export will disagree |
| PR #429 (open) | Audio track selection for transcripts and captions | Planned | Opened after the recorded baseline. Blocked behind a prerequisite this port does not have: there is no transcription or caption pipeline yet (#39, #91, #252, #287 are all Planned), so there is nothing to target. The transferable part is the argument contract â€” a positional `trackIndex` validated once and resolved to a stable track id internally â€” which matches the convention already used here and is the same class of hardening as refusing a clip placement onto an unknown track |
...
| PR #430 | Appearance settings: workspace layouts, adaptive light mode, timeline clip colors | Partial | The workspace half is adopted. `shared/ui/workspace-layout.ts` is the single definition of the three arrangements upstream ships â€” `default` (`[Media \| Preview \| Inspector] / [Timeline]`), `media` (media full height down the left, for sifting a large bin) and `vertical` (a tall preview column, which is what makes a 9:16 frame usable instead of letterboxed into a wide box) â€” together with their labels and the `Ctrl+1/2/3` chords that mirror upstream's `Cmd+1/2/3`. The store, the shortcut catalogue and the title bar switcher all read that one table, so a preset cannot be listed in the menu without a binding or vice versa. The choice is persisted and **narrowed on read**, matching upstream's `flatMap(init(rawValue:)) ?? .default` when loading from `UserDefaults`: a stored preset written by a build that no longer has it must not become the active layout. The Agent panel is deliberately a **sibling column of the preset rather than a member of it**, so switching arrangement never moves or tears down an in-progress chat. Presets stay orthogonal to the #286 visibility toggles â€” the preset decides where things sit, the toggles decide which of them are present â€” and a test pins that switching preset does not touch the panel object. One translated difference: upstream *collapses* an `NSSplitViewItem` and the split view hands its space to the remaining siblings, whereas hiding a panel here removes it from the tree, so the `vertical` preset lets the timeline take the whole left column when neither side panel is showing rather than leaving a hole under it. Missing: the adaptive light theme and dark-mode-by-default toggle (this port is dark-only with a warm accent, so a light theme means auditing every token, not adding a switch), customizable timeline clip colors, and the adaptive light theme (the splitter half of that gap is now closed â€” see the #286 row) |
| PR #440 | Make Swift package development traits opt in | N/A platform | The change makes `BundledSpeech` and `ProductionTelemetry` opt-in Swift Package Manager *traits* so a debug bundle skips compiling the MLX metallib, and forces them on for release. Every moving part is Apple-only: SPM traits, `swift build --traits`, the MLX Metal library, and `.app` bundle resource layout. There is nothing to translate, not merely nothing convenient â€” this port has no speech or telemetry subsystem to make optional, and its one heavy native component (the Rust/wgpu compositor) is already built by a separate `npm run build:rust` rather than by the renderer build |
| #453 | Media import silently drops files | Implemented | Both reported gaps are closed. `main/media/import-expansion.ts` classifies every dropped path before probing: a directory now expands recursively (depth â‰¤ 8 like the relink walk, 500-file ceiling with a single truncation notice, symlinks never followed so a junction cycle terminates by rule) and its supported media imports, where before the folder itself was refused as "not a supported media file" and none of its contents imported. A folder that cannot be listed reports `Could not read folder X` instead of vanishing, and explicitly picked files are admitted even after a folder hit the ceiling â€” the cap bounds expansion work, not the user's own selection. Unsupported files *inside* an expanded folder are ignored quietly (sidecars such as `.srt`/`thumbs.db` are not import failures) while top-level unsupported drops keep the named refusal. `shared/media/import-summary.ts` formats the full skip list for both import surfaces â€” first three reasons plus `(+N more)` â€” replacing `errors[0]`, so dropping several unsupported files names all of them (`import-expansion.test.ts`: recursion, bounds, unreadable folder via injected io, link skipping, dedupe; `import-summary.test.ts`) |
| #286 | Restructure the workspace panels | Partial | The request is CapCut-style layout control: rearrange the panels, detach the chat into its own window, or reduce the view to just the timeline and video. Most of it now works. Panel visibility already existed in the title bar; the layout is persisted in the UI store, so a reduced workspace survives a restart rather than being rebuilt every session, and stored flags are narrowed on read so a panel the saved layout omits falls back to its default. Rearranging arrived with the named presets adopted from PR #430, which is the shape upstream chose over free-form dragging and the one that actually answers the vertical-video case. **The resizable splitters are in**: every boundary between workspace regions is a draggable divider (`Divider` in `App.tsx`, pointer-captured so drags survive leaving the strip) owning exactly one stored dimension â€” media width, inspector width, the vertical preset's preview column, and the timeline height â€” streamed clamped into `ui.splits` and persisted under `palmier.layout.splits`; stored values are narrowed on read like every other layout key, defaults equal the recorded first-run rendered matrix so enabling dragging changes nothing until used, double-click restores a divider's default, and the pressure-shrink floors still win over an explicit basis so nothing can be dragged out of the window. Upstream gets this from `NSSplitView` autosave names; here it is the same persisted-store contract with tests (`ui-splits.test.ts`). Missing: tab grouping and detaching a panel into a separate window (`ui-panels.test.ts`, `ui-layout.test.ts`) |
| #166 | Preview must honour the project aspect ratio; move export to a workspace panel | Implemented | Both halves are done. Preview reads the live canvas from project settings and re-sizes with it, and the compositor composites at the project canvas, so a ratio change is reflected immediately. Export now docks as a dedicated right-hand workspace panel: `ExportDialog` became `ExportPanel`, riding the persisted panel flags as a fourth key (title-bar toggle with active state, Ctrl+M, Escape-to-hide) so settings and delivery history stay reachable while a render runs â€” the capability a modal structurally could not offer. Panel-scoped effects became mount-scoped, event subscriptions live only while docked, and shortcuts no longer treat export as modal-blocking. Fixed in passing: the captions checkbox block was duplicated verbatim, rendering two identical toggles |
...
| #58, #302 | Agent-driven multi-step edit stability | Partial | Editor tools are bounded pure state operations; decode, export, and generation run as separate processes off the IPC reply path; frame requests coalesce newest-wins (`main/media/latest-request.ts`); numeric arguments are range-checked (#264); the agent tool loop is capped at `MAX_TOOL_ROUNDS`. A turn can now be **stopped**: Send becomes Stop while one is running, `ai:cancel` aborts the in-flight request, and both provider paths check the signal between rounds and before each remaining tool call, so a response asking for six edits stops at the one the user interrupted. A tool already executing is allowed to finish, because the executor mutates through undoable commands and tearing one down halfway is how a timeline reaches a state no single undo reverses. Cancelling is reported as a stop, not an error, and clearing the transcript cancels first. **Anthropic history assembly was rebuilt**: a round is now exactly one assistant turn carrying every block the model produced, then one user turn carrying a tool_result for each tool_use in order. The assistant turn used to be appended once per tool and stored by reference, so a response asking for two tools declared both in the first assistant turn while the following user turn answered only the first â€” which Anthropic rejects, meaning any multi-tool response broke the turn on its next round. That is the shape of failure #58 describes, since a broken round is retried against a conversation that keeps growing. A stopped turn answers the tools it did not run with a cancelled error result rather than leaving them unanswered, and a later message joins the trailing results turn instead of adding a second consecutive user turn, so neither a stop nor the round cap can make every subsequent message fail. A long multi-step turn is covered by a stress suite that pins replayed history to linear growth in the number of rounds â€” superlinear growth is the mechanism behind the reported stall, because every round re-sends the whole conversation â€” plus project consistency across dozens of mutations, undoability of the entire run, and clean handling of a run of entirely invalid tool calls. #302's `manage_tracks` mis-targeting has no direct analogue (there is no such tool, and every track-addressing tool takes a stable track id rather than an index, which is the fix upstream landed in PR #307), but the same class of defect existed and is now closed: `addClip` accepted a track id that did not exist, producing a clip invisible to the timeline, the compositor and the exporter while still counting in the clip list, and reporting success to the caller. Missing: a headless mode for batch MCP production (`agent.anthropic.test.ts`, `agent.stress.test.ts`, `agent.openai-compatible.test.ts`, `openai-compatible.test.ts`, `executor.add-clip.test.ts`) |

### v0.7.0 â€“ v0.7.6 triage (baseline moved 2026-08-21)

Rows below cover everything upstream merged between `8d5648d8` and `8805801f`
(~100 commits, 7 releases). Grouped rather than one row per commit where a
cluster shares a disposition.

| Upstream | Area | Windows disposition | Windows owner / evidence |
|---|---|---|---|
| PR #471 | Preserve omitted OpenAI tool arguments | Implemented | `toOpenAiTools` pins `strict: false` on every function tool, and `parseToolArguments` drops top-level keys sent as explicit `null` before Zod validation â€” this port's translation of upstream's Responses-API fix, since strict normalization (or a quirk-compatible endpoint) otherwise turns "model omitted this optional" into "explicit null", which every schema reads as a validation failure instead of "use the default" (the `remove_silence` saved-controls contract). Nested objects are untouched; schemas are flat (`openai-compatible.test.ts`) |
| PR #544 | Prevent muted audio from stalling exports | Implemented | Track-level mute was already a build-time exclusion here (the exporter filtered on `track.visible !== false`, matching upstream's fixed invariant), but per-clip `muted` was ignored, so a muted audio clip still reached the export mix at full volume while preview muted it â€” a preview/export eligibility disagreement in exactly the class #544 fixes. `shared/media/export-eligibility.ts` now owns one list consumed by both the extent calculation and the FFmpeg argument builder: hidden tracks excluded, individually muted audio clips excluded, video unaffected by the audio-only flag (`export-eligibility.test.ts`) |
| PR #486 | Index clip lookups by id | Implemented | The controller resolved ids by linear scan at every mutation entry point (`moveClip`, `trimClipEdge`, `splitClip`, `removeSilence`). A lazily built idâ†’clip map keyed to the exact project object it was built from gives O(1) lookups with reference identity as the invalidation signal â€” sound because every command, undo, redo, and restore replaces `this.project` wholesale and no path mutates clip objects in place. First occurrence wins per id, matching the `Array.find` it replaced (`controller.clip-index.test.ts`: correctness across mutation/undo/redo/remove/add, no stale identity) |
| PR #488 | Preserve drag access on narrow timeline clips | Implemented | Same defect existed here: 6 px trim zones on clips rendered as narrow as 4 px overlapped and covered the whole body, so short clips could be trimmed but never grabbed to move. `renderer/lib/timeline-clip-hit.ts` owns the rule â€” below 16 px rendered width the hit zone is always `body` and the visual handles do not render â€” with the thresholds and zone math unit-tested; `TimelineClip` consumes it. Rendered matrix not re-run for this change: no layout-affecting markup changed, only conditional handle rendering and mousedown routing (`timeline-clip-hit.test.ts`) |
| PR #487 | Batch clip removal in moveClips | Implemented (by construction) | No port needed: `removeClips` filters once per call and `moveClip` rewrites the clips array in a single `.map()` pass; there is no per-clip lookupâ†’mutateâ†’lookup cycle for the upstream fix to remove |
| PR #537 | Prevent full-timeline mutation receipt encoding | Different by design | Mutation receipts here were already shaped summaries ({moved}, {trimmed}, ripple reports) â€” no code path ever serialized the whole timeline into a tool result, so the defect class is unreachable. `get_timeline` remains the only full dump, by explicit request |
| PRs #542, #560, #552/#551, #504 | Timeline markers + marker review/index surfaces | Partial | The **domain layer is ported and upstream-exact**: `shared/editor/markers.ts` holds the model (`TimelineMarker {id, name â‰¤120 single line, startFrame, durationFrames (0 = point), #RRGGBB[AA] color defaulting to upstream's blue, comment â‰¤4000}`), validation, canonical `(startFrame, id)` ordering, fps-rescale, and the ripple engine â€” a faithful translation of `RippleEngine.rippleMarkers` including its two embedded fixes: empty hole lists do not vote, and surviving positions take the minimum mapped start/end so a marker is never dragged further than one cut requires. `EditorController.changeTimelineMarkers` does create/update/delete mixes as **one undoable step** (`ReplaceMarkersCommand`, whole-array swap like upstream's `registerTimelineMarkerSwap`) with precise thrown errors and no-op calls adding no history entry; all four ripple transactions (`rippleDeleteClips`, `rippleDeleteGap`, `rippleDeleteRanges`, ripple `trimClipEdge`) remap markers in the same transaction, and `applyProjectSettings` rescales them with the timebase. Persistence rides the optional `Timeline.markers` key, so pre-marker projects decode unchanged. The Agent/MCP surface exists: `manage_markers {action: create|update|delete, â€¦}` with per-action argument requirements and no-op receipts, and `get_timeline` now returns markers (`markers.test.ts`, `controller.markers.test.ts`, `controller.marker-ripple.test.ts`, `executor.markers.test.ts`). **The visual surface is implemented**: point markers render as pennant flags and range markers as bands in the ruler (`TimelineRuler`), click selects (Shift additive, dismissing clip/gap selections), drag moves with a local preview and exactly one undo commit per drag, double-click renames inline; `M` adds an auto-named "Marker N" at the playhead (collision-skipping), Delete removes selected markers *before* clips fall through, Escape clears the marker selection, and marker edges are snap targets through `getSnapPoints` â€” finally giving the reserved `'marker'` snap source its meaning (`timeline-markers.test.ts`). Missing: the full editor popover (color/comment/duration fields â€” the Agent tool covers them), range-marker creation from the UI, the timeline-index review browser (#552/#504), and the `rippleTimelineMarkers` preference toggle (Windows ripples markers unconditionally, matching upstream's default-on) |
| PRs #535, #539, #540, #541 | Keyframe lanes; animated text size and blur; audio keyframe controls | Partial | **Position keyframes (v1) are live end to end**: `shared/media/motion.ts` owns sanitize + linear evaluation; motionX/motionY on video/image clips drive the canvas transform, the GPU descriptor x/y, and - exactly, via piecewise-linear overlay arithmetic in output seconds (motionExpression) - the export graph. Agent tool set_clip_motion sanitizes/clears per axis; Inspector Motion section captures/updates/removes keyframes at the playhead with a per-axis easing selector. Easing curves beyond linear are IN: linear/easeIn/easeOut/easeInOut, evaluated identically in canvas and emitted exactly in export via `if`/`pow` over normalized segment time (motionExpression). **Scale keyframes (sx/sy) are now live end to end**: `motionScaleX`/`motionScaleY` on Clip type; Canvas compositor evaluates per-frame scale via `evaluateMotion`; export uses FFmpeg `scale` filter with time-based expressions for animated scale; `set_clip_motion` agent tool accepts `sx`/`sy` axes; Inspector renders Scale X/Y keyframes with easing. **Rotation keyframes (r) are now wired in preview**: `motionRot` was already in export but not in canvas compositor — now uses `evaluateMotion(clip.motionRot, currentFrame)` for per-frame rotation. **Audio keyframes (#541's slice) are now live end to end**: `shared/audio/volume-keyframes.ts` owns a `volumeDb` automation track reusing the same sanitize/evaluate/expression engine as the position/scale/rotation tracks, in absolute decibels (-60 floor, +15 ceiling) matching upstream's `VolumeScale`, with an active track authoritative over the static `volume` field per upstream's PR #522 fix (first-keyframe level jump). Wired into live playback (`audio-playback.ts`, a new `GainNode` in `audio-preview.ts` for boost above unity), FFmpeg export (`volumeFilterExpression` in the export audio chain), the `set_clip_volume_keyframes` agent tool, and an Inspector automation section. Still missing: animated text size, a visual lane editor |
| PRs #518, #511, #510, #506, #476, #479, #490, #557, #564, #513 | Captions/transcripts cluster (SRT/VTT import, caption preview, length limits, outline/gap/highlight fixes, transcript-first index, cancellation) | Partial | **#91's core planning logic is now implemented** as `shared/captions/planner.ts`: planCaptions(wordTimings, {maxCharsPerLine, maxLines, pauseBreakSec}) snaps cue boundaries to real word timestamps (never character-count distribution), breaks at natural pauses and sentence punctuation, and simulates actual greedy line packing for its budget decisions so the <=N-lines invariant holds by construction - engine-agnostic groundwork that any transcription source feeds into (planner.test.ts). What remains is everything downstream of transcription itself - transcript-first index (#506), generation timing (#511/#510), outline/gap/highlight refinements (#476/#479/#490/#557/#564/#513) - blocked by the absent STT subsystem (#39/#252/#287) |
| PRs #482, #494, #509, #507, #525, #529, #530, #519, #524 (+ #330 line spacing/font case) | Text styling cluster | Partial | The text domain this cluster assumed absent has landed: `shared/editor/title.ts` owns sanitization plus a style model (size ratio, color, font family, bold, alignment, background with alpha + padding, stroke width/color) rendered identically by canvas preview and FFmpeg drawtext, edited inline on the clip and from the Agent via `add_texts`/`set_title_text`. Covered so far: default-style centralization (#482 via `DEFAULT_TITLE_STYLE`); alignment (#509); fitted boxes with agent-exposed padding (#507, canvas rects + drawtext `boxborderw`, font-before-measure so both paths wrap the same glyphs); line spacing + font case adopted from #330's agent-tool styling (`titleLineSpacing` â†’ drawtext `line_spacing`; case is a string transform applied before both renderers, the strongest form of paths-agree); and **footage fill, inverted fill, and text blur (#525/#529/#530) via the bake layer specified in [`TEXT_BAKING_DESIGN.md`](./TEXT_BAKING_DESIGN.md)** â€” one shared renderer (`renderer/engine/title-render.ts`) draws preview and bake alike; advanced clips ship full-canvas RGBA PNGs through `export:bake-titles` and composite as `overlay` (knocked-out band), `blend=all_mode=difference` (inverted silhouette), or an in-bake blur, with alpha fades, clip opacity, looping still inputs, per-export temp cleanup, and a degrade-to-solid fallback when no bake exists; and **perspective tilt (#519)** â€” `titleTiltXDeg`/`titleTiltYDeg` project the layer through a faithful port of upstream's `TextTiltGeometry` (pivot-anchored pinhole with rect-independent focal length), composited in canvas via strip interpolation and riding the same bake pipeline for export. All nine PRs are accounted for. **#494 (preview rebuild preservation) is Different by design**: upstream's async full-rebuild completed against a stale snapshot and could overwrite newer text styling, requiring a generation-tracked corrective visual pass; this port has no long-lived composition snapshot at all - every frame request re-reads the live project (setProject per invocation), edits mint a fresh render-cache token that orphans stale frames instantly (#546 mechanism), the newest-request gate drops superseded sends, and an edit landing mid-decode affects only that single in-flight frame before the next request recomposites under current values. The persistent-staleness window #494 fixes cannot form here |
| PRs #431, #455, #445â€“#452, #469, #561 | Localization foundation + nine languages + agent chat localization | Planned | Real product behavior upstream (a full i18n stack), not docs. Adopting it means an extraction pass over every user-facing string in renderer and main; deferred as its own workstream, not skipped |
...
| PRs #497, #534 | Community skill sync; in-app skill management | Planned | No skills subsystem exists. #553 (Smart Search model on demand) is N/A platform â€” it installs an MLX model |
| PRs #456, #458 | Onboarding tour and guided flow | Planned | Product surface, applies in full; sequenced after parity-critical work |
| PRs #498, #481, #499 | FLUX.3 workflows, MiniMax H3 reframe, Fish Audio logo | Planned | The specific model catalogs/workflows are not in the built-in provider lists; the tool itself is live (`generate_media` over the fal/Replicate/HiggsField registry), so adopting these is now a catalog entry per provider rather than plumbing |
...
| PR #546 | Reuse source assets during composition builds | Implemented | Both halves of the reuse concern are covered. Preview already deduped decodes through the frame LRU keyed by asset+size+time with concurrent-request coalescing. Export now consolidates too: `main/media/export-args.ts` â€” the argument builder extracted out of `exporter.ts` into a pure, Electron-free module so it is unit-testable â€” assigns exactly **one `-i` input per unique source path** in first-use order and remaps every filter chain (`[n:v]trim=â€¦`) and audio `-map` to the consolidated index, where before N clips from one source spawned N full decodes. FFmpeg fans one input out to multiple filter chains, so per-clip trim/scale/overlay semantics are unchanged (`export-args.test.ts`: consolidation, index remapping, canvas-at-0 ordering, muted-audio exclusion interplay, no-addon fallback). The consolidation pass also **closed the long-standing audio-timing gap**: eligible audio clips now build per-clip chains through the same source-time mapping as video (`atrim` + `asetpts`), with timeline-position delay (`adelay`), clip volume (`volume`), and an `amix=normalize=0` mix mapped once as `[aout]` â€” previously every audio clip exported as a raw full-source stream from time zero regardless of placement. Remaining, inherent to the per-clip-trim filter design: FFmpeg still decodes the shared input once per referencing chain at different offsets; audio fades are not yet an audio-side ramp (the fade fields are visual-opacity semantics in this port) |
| PR #520 | Editable track names + `manage_tracks` | Implemented | Both halves are ported. Naming: `shared/editor/track-name.ts` owns upstream's rules (trim, refuse control characters/newlines, â‰¤80 chars, empty restores the generated `Video N`/`Audio N` label from the track's position among its type); `EditorController.setTrackName` applies it as one undoable step, refusing invalid or unchanged input without a history entry; the track header renames inline on double-click or via its context menu (`track-name.test.ts`). The Agent surface is upstream-exact: `manage_tracks {reorder?, set?, remove?}` resolves every entry by exactly one of `trackId`/current `index` (the #302 mis-targeting class), applies reorders â†’ sets â†’ removals as **one atomic undoable step**, confines reorder destinations to the track's type zone (orders renumbered per zone so untouched zones keep their exact values and list head stays the top layer), folds `muted`/`hidden` onto this port's single `visible` toggle, treats a present `name: ""` as clear-to-generated with absent meaning untouched, refuses non-empty-track removals and the last of either type, reports `{tracks, reordered?, renamed?, removedTracks?}` receipts, and adds no history for no-ops (`controller.managetracks.test.ts`, `executor.managetracks.test.ts`). Deliberate supersets: `remove` also accepts a bare track-id string, and refusals carry precise reasons |
...
| Issue #556 | Playback slow to start on sparse timelines | Implemented | Investigated with a repro, then fixed: the compositor's per-frame layer scan paid O(clips Ã— tracks) (a track `find` per clip in the filter, two per sort comparison). `main/media/visible-clips.ts` resolves layers in one indexed pass â€” same semantics, O(clips + tracks) â€” and the per-pass media lookup is indexed too. Measured on 40 tracks / 3000 clips over 120 resolutions: worst-case placement 120.9 ms â†’ 8.65 ms (14Ã—), typical 2Ã—; the cost no longer grows with track count. Upstream's 50-second magnitude never reproduced here (bounded decode pool + newest-wins coalescing); regression guard in `visible-clips.test.ts` |
...
| PRs #570, #565, #568 | Generation cost display; inspect-frame grid overlay; static crop in set_clip_properties | Implemented | **inspect_frame (#565) is live**: `{assetId, atSeconds, width?}` decodes through the shared frame decoder, encodes RGBA→PNG via a temp-file ffmpeg handoff, stores under userData/inspect-frames, and returns absolute path + dimensions + timecode; over MCP the PNG attaches as a native image content block so vision clients see the frame directly. **Static crop (#568) is live**: `Clip.crop` edge fractions (capped 0–0.45/pair ≤0.9 via `shared/media/source-crop.ts`) applied as a proportional sub-rect of the decoded RGBA in preview — pixel-equivalent to source-space crop through uniform scale — and an FFmpeg `crop` filter ahead of scale on export; agent tool `set_clip_crop` sanitizes and clears on all-zeros. #570 generation cost display is live: `MediaAsset.generatedBy` carries {provider, model, costCredits?} and is populated by the executor on generation import; the Inspector renders a "Generated" section with provider/model and cost when the clip comes from an AI-generated asset. Cost is surfaced from the provider result when available (fal.ai, Replicate may include usage billing in their responses), left blank for providers that do not report credits, and persisted through the project serialization so cost survives a project reopen ( `shared/types/project.ts`, `renderer/components/Inspector.tsx`, `main/ai/executor.ts`, `main/generation/types.ts`) |
| PRs #569â€“#572 | Agent generation model recommendations (Seedance 2.5, GPT Image, Seedream, MiniMax H3) | Implemented | Agent system prompt now recommends Seedream/GPT Image for images, Seedance 2.5/MiniMax H3 for video, and meta/musicgen for audio, with a fallback-to-default instruction when a preferred model is unavailable (`main/ai/agent.ts`) |
...
Last run: 2026-08-22, via the committed probe (`npm run ui:probe`) â€” an
...
This run covers the 2026-08-21 UI batch â€” track-header context menu and
...
breakpoint was later found to be measuring the wrong thing â€” see the third run
below.
...
rightmost controls were clipped out of reach again â€” the same failure the 1200 px
...
more than the row had, and the rightmost panel was simply outside the window â€” the
...
taste â€” that row needs about 290 px at its narrowest container tier, so a 300 px
