...
| [#91](https://github.com/palmier-io/palmier-pro/issues/91) | Captions: timing drift + no words-per-caption control | Planned | A caption *surface* now exists — SRT/VTT import lands cues as title clips, rendered in preview and exported as a WebVTT sidecar — but the complaint upstream describes is about generated captions, which require the absent transcription subsystem (#39). The lesson to carry over stands: snap caption phrases to real word timestamps rather than distributing by character count. |
...
| [#252](https://github.com/palmier-io/palmier-pro/issues/252) | Sharing an idea for caption transcription | Planned | Blocked on the absent transcription subsystem (#39) only — the caption display surface it targets now exists (SRT/VTT import as title clips, WebVTT sidecar export). |
